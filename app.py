...
# ============================================================================
    ...
    metodo = st.radio(
        ...
        ["📊 Ingresar datos crudos", "📐 Ingresar estadísticos (n y s²)"],
        horizontal=True
    )
    ...
    else:  # Estadísticos calculados
        st.info("💡 Ingresa el tamaño de muestra (n) y la varianza muestral (s²)")
...
# ============================================================================
    ...
    if metodo == "📊 Ingresar datos crudos":
        ...
        with col2:
            ...
            alpha = st.slider("Nivel de significancia (α):", 0.01, 0.10, 0.05, 0.01, key="alpha_ph")
        ...
        if st.button("Realizar Prueba", key="ph"):
            try:
                ...
                if len(datos) < 2:
                    ...
                    resultado = pruebas.prueba_hipotesis_varianza(datos=datos, varianza_h0=varianza_h0, hipotesis=hipotesis, alpha=alpha)
        ...
        with col1:
            n_input = st.number_input("Tamaño de muestra (n):", min_value=2, value=10, step=1, key="n_ph_calc")
            var_input = st.number_input("Varianza muestral (s²):", min_value=0.0001, value=0.5, step=0.01, format="%.4f", key="var_ph_calc")
        
        with col2:
            varianza_h0 = st.number_input("Varianza bajo H₀ (σ₀²):", value=0.5, step=0.1, format="%.4f", key="var_h0_calc")
            hipotesis = st.selectbox("Tipo de hipótesis:", ["bilateral", "mayor", "menor"], key="hip_calc")
            alpha = st.slider("Nivel de significancia (α):", 0.01, 0.10, 0.05, 0.01, key="alpha_ph_calc")
        
        if st.button("Realizar Prueba", key="ph_calc"):
            try:
...
# ============================================================================
    ...
    metodo = st.radio(
        "¿Cómo deseas ingresar la información?",
        ["📊 Ingresar datos crudos", "📐 Ingresar estadísticos (n₁, s₁², n₂, s₂²)"],
        horizontal=True,
        key="metodo_f"
    )
    ...
    if metodo == "📊 Ingresar datos crudos":
        ...
        if st.button("Realizar Prueba F", key="pf"):
            try:
                ...
                if len(datos1) < 2 or len(datos2) < 2:
                    ...
                    resultado = pruebas.prueba_dos_varianzas(datos1=datos1, datos2=datos2, hipotesis=hipotesis, alpha=alpha)
        ...
        st.info("💡 Ingresa los tamaños de muestra y varianzas muestrales de ambos grupos")
        
        col1, col2 = st.columns(2)
        
        with col1:
            st.subheader("Muestra 1")
            n1_input = st.number_input("Tamaño de muestra (n₁):", min_value=2, value=10, step=1, key="n1_calc")
            var1_input = st.number_input("Varianza muestral (s₁²):", min_value=0.0001, value=0.5, step=0.01, format="%.4f", key="var1_calc")
        
        with col2:
            st.subheader("Muestra 2")
            n2_input = st.number_input("Tamaño de muestra (n₂):", min_value=2, value=12, step=1, key="n2_calc")
            var2_input = st.number_input("Varianza muestral (s₂²):", min_value=0.0001, value=0.8, step=0.01, format="%.4f", key="var2_calc")
        
        col1, col2 = st.columns(2)
        with col1:
            hipotesis = st.selectbox("Tipo de hipótesis:", ["bilateral", "mayor", "menor"], key="hip_f_calc")
        with col2:
            alpha = st.slider("Nivel de significancia (α):", 0.01, 0.10, 0.05, 0.01, key="alpha_f_calc")
        
        if st.button("Realizar Prueba F", key="pf_calc"):
            try:
                resultado = pruebas.prueba_dos_varianzas(
                    n1=n1_input,
                    n2=n2_input,
                    var1=var1_input,
                    var2=var2_input,
                ...
                ax.plot(x, y, 'b-', linewidth=2, label=f'F({resultado["gl1"]}, {resultado["gl2"]})')
                ax.axvline(resultado['F_calculado'], color='red', linestyle='--', linewidth=2, label='F calculado')
